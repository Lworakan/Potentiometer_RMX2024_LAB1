...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) potenlab_M->Timing.taskTime0;
  potenlab_step();
...
int main(int argc, char **argv)
{
  ...
  MX_ADC1_Init();
  rtmSetErrorStatus(potenlab_M, 0);
  ...
  errorCode = extmodeInit(potenlab_M->extModeInfo, &rtmGetTFinal(potenlab_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(potenlab_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(potenlab_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
